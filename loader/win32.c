...
{
    void *ret = my_mreq(cb, 0);
    dbgprintf("CoTaskMemAlloc(%u) => %08x\n", cb, ret);
    return ret;
}
static void WINAPI expCoTaskMemFree(void* cb)
{
    dbgprintf("CoTaskMemFree(%08x)\n", cb);
    my_release(cb);
}
...
static int WINAPI explstrlenW(LPWSTR str1)
{
    LONG len = 0;
    while(str1[len]) len++;
...
struct exports exp_kernel32[]=
{
    ...
    FF(GetLastError, -1)
    FF(SetLastError, -1)
    REMAP(RestoreLastError, -1, SetLastError)
